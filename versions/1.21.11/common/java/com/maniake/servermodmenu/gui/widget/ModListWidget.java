...
if(isInit) {
	for (int index = 0; index < entryCount; ++index) {
		...
		entry.renderContent(DrawContext, mouseX, mouseY, this.isMouseOver(mouseX, mouseY) && Objects.equals(this.getEntryAtPos(mouseX, mouseY), entry), delta);
	}
}
	}
...
//	}
	...
	public int getRowWidth() {
		return this.width - (Math.max(0, (this.children().size() * this.contentHeight() + this.defaultEntryHeight + 4) - (this.getBottom() - this.getY() - 4)) > 0 ? 18 : 12);
	}
